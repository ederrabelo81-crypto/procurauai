...
/* ─────────────────────────────────────────────────────────────────────────
   Street View Static API — fachada do comércio quando não há foto própria
   ───────────────────────────────────────────────────────────────────────── */

/**
 * Imagem da fachada a partir das coordenadas. Devolve null sem chave.
 *
 * Serve de último recurso visual para comércios ainda sem foto enviada pelo
 * comerciante — imagem servida pelo próprio Google, sem cópia de conteúdo.
 * Exige a **Street View Static API** habilitada no projeto do Cloud Console
 * (é um produto separado da Maps Static API).
 *
 * `return_error_code` faz o Google responder 404 em vez de devolver a imagem
 * cinza "Sorry, we have no imagery here", para o `onError` do `<img>` cair no
 * placeholder em vez de exibir o aviso do Google.
 */
export function mapsStreetViewUrl(
  target: LatLng | string,
  {
    width = 640,
    height = 360,
    fov = 80,
    pitch = 0,
  }: { width?: number; height?: number; fov?: number; pitch?: number } = {},
): string | null {
  if (!hasMapsKey) return null;
